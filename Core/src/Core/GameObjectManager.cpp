...
void Core::GameObjectManager::Update(const double& p_deltaTime)
{
    ...
    Find("Torus")->GetComponent<Components::TransformComp>()->m_transform->Rotate(glm::vec<3, double>(1, 0, 0) * p_deltaTime);
    Find("Gear")->GetComponent<Components::TransformComp>()->m_transform->Rotate(glm::vec<3, double>(0, 1, 0) * p_deltaTime);
}
